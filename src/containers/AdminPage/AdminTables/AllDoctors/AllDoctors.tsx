...
const AllDoctors: React.FunctionComponent = (): React.ReactElement => {
    ...
    const {
        ...
        isLoading,
    } = useGetDoctorsQuery({ offset: 0, limit: 10 });
    const [blockThisUser, { error: blockUserError, isError: isBlockError }] =
        useBlockDoctorMutation();
    const [activateThisDoctor, { error: activateError, isError: isActivateError }] =
        useActivateDoctorMutation();
    ...
    const cklickBlockHandler = (
        e: React.MouseEvent<HTMLElement>,
        thisDoctor: IDoctorWithUser,
        text: string
    ) => {
        e.stopPropagation();
        ...
        <div className={styles.list_container}>
            <div className={styles.group_row}>
                ...
                <div className={styles.add_link_colappsed}>
                    <NavLink
                        className={`${styles.add_icon} ${styles.add_doctor_icon}`}
                        to={"/admin-page/register-doctor"}
                    ></NavLink>
                </div>
                <div className={styles.collapsing_link}>
                    <TransparentLink
                        title={"Зарегистрировать врача"}
                        size={EBtnSize.tiny}
                ...
                <div className={styles.modal_flex_column}>
                    <div className={styles.title_box}>
                        <p className={styles.modal_title}>
                            ...
                            <span className={styles.violet}>{modalTitle}</span> пользователя по
                            имени
                            <span className={styles.violet}>
                                {stateDoctor &&
                                    ` ${stateDoctor.users.surname} ${stateDoctor.users.name}`}
                            </span>
                            ?
                        </p>
                    </div>
                                ...
                                {doctors.map(({ ...doctor }) => {
                                    return (
                                        <tr
                                            ...
                                            key={doctor.id}
                                            onClick={(e) =>
                                                navigateCabinetHandler(e, doctor.userId)
                                            }
                                        >
                                            <td className={styles.Table_td_right}>
                                                {doctor.users.surname} {doctor.users.name}{" "}
                                                {doctor.users.patronim ? doctor.users.patronim : ""}
                                            </td>
                                            <td className={styles.Table_td_right}>
                                                {doctor.users.email}
                                            </td>
